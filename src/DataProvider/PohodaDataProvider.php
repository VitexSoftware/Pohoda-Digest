...
class PohodaDataProvider implements DataProviderInterface
{
    ...
    public function getData(string $entity, array $conditions = [], array $columns = []): array
    {
        ...
        $raw = match ($entity) {
            DataProviderInterface::ENTITY_OUTCOMING_INVOICES => self::fetchInvoices($pohodaFilter, 'issuedInvoice'),
            DataProviderInterface::ENTITY_INCOMING_INVOICES => self::fetchInvoices($pohodaFilter, 'receivedInvoice'),
            DataProviderInterface::ENTITY_BANK_STATEMENTS => self::fetchBankTransactions($pohodaFilter),
            DataProviderInterface::ENTITY_CONTACTS => self::fetchContacts($pohodaFilter),
            DataProviderInterface::ENTITY_PRODUCTS => self::fetchProducts($pohodaFilter),
        ...
        foreach ($conditions as $key => $value) {
            switch ($key) {
                ...
                case DataProviderInterface::FILTER_CANCELLED:
                    $want = (bool) $value;
                    $postFilters[] = static fn (array $r): bool => ($r[DataProviderInterface::FIELD_CANCELLED] ?? false) === $want;
                ...
                case DataProviderInterface::FILTER_PAYMENT_STATUS:
                    ...
                    $postFilters[] = static function (array $r) use ($want): bool {
                        ...
                        return match ($want) {
                            DataProviderInterface::PAYMENT_STATUS_UNPAID_OR_PARTIAL => $status === DataProviderInterface::PAYMENT_STATUS_UNPAID
                ...
                case DataProviderInterface::FILTER_PAYMENT_DIRECTION:
                    $want = (string) $value;
                    $postFilters[] = static fn (array $r): bool => ($r[DataProviderInterface::FIELD_DIRECTION] ?? '') === $want;

                    break;
                case DataProviderInterface::FILTER_MATCHED:
                    $want = (bool) $value;
                    $postFilters[] = static fn (array $r): bool => ($r[DataProviderInterface::FIELD_MATCHED] ?? false) === $want;

                    break;
                case DataProviderInterface::FILTER_ACCOUNTED:
                    $want = (bool) $value;
                    $postFilters[] = static fn (array $r): bool => ($r[DataProviderInterface::FIELD_ACCOUNTED] ?? false) === $want;

                    break;
                case DataProviderInterface::FILTER_MISSING_EMAIL:
                    if ($value) {
                        $postFilters[] = static fn (array $r): bool => ($r[DataProviderInterface::FIELD_EMAIL] ?? '') === '';
                    }

                    break;
                case DataProviderInterface::FILTER_MISSING_PHONE:
                    if ($value) {
                        $postFilters[] = static fn (array $r): bool => ($r[DataProviderInterface::FIELD_PHONE] ?? '') === '';
                    }

                    break;
                case DataProviderInterface::FILTER_EXCLUDE_DOCUMENT_TYPE:
                    $exclude = (string) $value;
                    $postFilters[] = static fn (array $r): bool => ($r[DataProviderInterface::FIELD_DOCUMENT_TYPE] ?? '') !== $exclude;
     ...
     */
    private static function fetchInvoices(array $filter, string $invoiceType = 'issuedInvoice'): array
    {
        try {
            $client = new Invoice();
     ...
     * @return array<array<string, mixed>>
     */
    private static function fetchBankTransactions(array $filter): array
    {
        try {
            $client = new Bank();
            $raw = $client->getColumnsFromPohoda(['*'], $filter);
     ...
     * @return array<array<string, mixed>>
     */
    private static function fetchContacts(array $filter): array
    {
        try {
     ...
     * @return array<array<string, mixed>>
     */
    private static function fetchProducts(array $filter): array
    {
        try {
    ...
    private static function normalizeInvoice(array $inv, string $invoiceType): array
    {
        ...
        $totalHome = self::parseAmount($homeCurrency);
        $totalForeign = self::parseAmount($foreignCurrency);
        $remaining = self::parseRemainingAmount($inv);
        ...
        return [
            ...
            DataProviderInterface::FIELD_CURRENCY => self::parseCurrency($homeCurrency ?: $foreignCurrency),
            DataProviderInterface::FIELD_CANCELLED => $state === 'cancelled',
            DataProviderInterface::FIELD_DOCUMENT_TYPE => self::normalizeDocumentType((string) ($inv['invoiceType'] ?? $inv['documentType'] ?? $invoiceType)),
    ...
    private static function normalizeBank(array $tx): array
    {
        ...
        $amount = self::parseAmount($homeCurrency);
        ...
        return [
            ...
            DataProviderInterface::FIELD_TOTAL_AMOUNT_FOREIGN => abs(self::parseAmount($tx['foreignCurrency'] ?? [])),
            DataProviderInterface::FIELD_CURRENCY => self::parseCurrency($homeCurrency),
     ...
     * @param array<string, mixed> $currencyData
     */
    private static function parseCurrency(array $currencyData): string
    {
     ...
     */
    private static function normalizeDocumentType(string $pohodaType): string
    {
